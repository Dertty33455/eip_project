...
export default function AudiobookDetailPage() {
  ...
  const fetchAudiobook = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Robustly normalize chapters — handle both camelCase and snake_case from API
        const normalizedChapters: Chapter[] = (data.chapters || []).map((ch: any) => {
          // is_free can be 0/1 (integer) or true/false (boolean) — normalize to boolean
          const isFreeRaw = 'isFree' in ch ? ch.isFree : ch.is_free
          return {
            id: String(ch.id),
            title: ch.title,
            duration: ch.duration,
            audioUrl: ch.audioUrl ?? ch.audio_url ?? '',
            order: ch.order ?? ch.chapter_number ?? 0,
            isFree: isFreeRaw === true || isFreeRaw === 1,
          }
        })

        const normalized: Audiobook = {
          id: String(data.id),
          title: data.title,
          author: data.author,
          ...
          reviewCount: data.reviewCount ?? data.reviews_count ?? 0,
          releaseYear: data.releaseYear ?? data.published_year ?? data.release_year ?? null,
          ...
          reviews: (data.reviews ?? []).map((r: any) => ({
            ...
            user: {
              ...r.user,
              firstName: r.user?.firstName ?? r.user?.first_name ?? r.user?.username ?? '?',
            }
          })),
          relatedAudiobooks:
            data.relatedAudiobooks ??
            data.related_audiobooks ??
            data.allRelated ??
            data.all_related ??
            [],
        }

        setAudiobook(normalized)

      } else {
        console.error('Error fetching audiobook:', res.statusText)
      }
    } catch (error) {
  ...
  // ✅ FIX: explicit boolean check — isFree is guaranteed boolean after normalization
  const canPlayChapter = (chapter: Chapter) => {
    return chapter.isFree === true || hasActiveSubscription === true
  }
  ...
  const relatedList = audiobook.relatedAudiobooks || []
  // Dynamically show free badge only if at least one chapter is free
  const hasFreeChapter = chapters.some(ch => ch.isFree)
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            >
              ...
              <div className="aspect-square bg-gradient-to-br from-primary/10 to-accent/10 relative">
                ...
                {/* Badge — only shown if there's actually a free chapter */}
                {hasFreeChapter && (
                ...
                <div className="flex items-center gap-4 mb-4">
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <FiStar
                        key={star}
                        className={`w-5 h-5 ${star <= Math.round(audiobook.rating)
                      ...
                      {chapters.map((chapter, index) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full flex items-center gap-4 p-4 rounded-xl transition-colors text-left ${isCurrent
                          ...
                          >
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2">
                                <span className={`font-medium ${isCurrent ? 'text-primary' : 'text-gray-800'
                                  }`}>
                                  {chapter.title}
                                </span>
                                {chapter.isFree && (
                    ...
                    >
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-500">{formatDuration(related.totalDuration ?? related.total_duration ?? 0)}</span>
                          <div className="flex items-center gap-1 text-sm">
                            <FiStar className="w-4 h-4 text-yellow-400 fill-current" />
                            <span className="text-gray-600">{related.rating ?? 0}</span>
                          </div>
                        </div>
                      </div>
                    </Link>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="hidden lg:flex items-center gap-4">
                        ...
                        >
                          {[0.5, 0.75, 1, 1.25, 1.5, 2].map((speed) => (
                            <button
                              ...
                              className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-50 ${playbackSpeed === speed ? 'text-primary font-medium' : 'text-gray-700'
                                }`}
                            >
                              {speed}x
                            </button>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-full transition-colors ${isRepeat ? 'text-primary bg-primary/10' : 'text-gray-600 hover:text-primary'
                      }`}
                  >
